...
def canonical_unit(unit: str) -> str:
    unit = re.sub(r'\s*/\s*', '/', unit.strip()).replace('µ', 'u')
    fixes = {'fl': 'fL', 'g/dl': 'g/dL', 'mg/dl': 'mg/dL'}
    return fixes.get(unit.casefold(), unit)

def _normalize_unit(unit: str) -> str:
    return canonical_unit(unit).casefold()
...
class LabRowValidationService:
    def validate(self,row:LabResultV2, unsafe_ocr:bool=False)->LabResultV2:
        if unsafe_ocr:
            ...
            return row
        if row.unit:
            row.unit = canonical_unit(row.unit)
