...
class CommonFieldValidationService:
    ...
    def _tracking(self,text):
        ...
        val=m.group(1) if m else None; ok=valid_tracking_number(val, bool(m))
    ...
    def _date(self,text):
        m=re.search(r'(?:تاریخ|Date|Report Date|Admission Date)\s*[:：]?\s*(1[34]\d{2}[/-](?:0?[1-9]|1[0-2])[/-](?:0?[1-9]|[12]\d|3[01]))', text, re.I)
