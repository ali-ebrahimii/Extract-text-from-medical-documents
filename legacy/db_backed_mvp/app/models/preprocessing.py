...
class DocumentPreprocessing(Base):
    ...
    preprocessing_required=Column(Boolean); preprocessing_applied=Column(Boolean); quality_score_before=Column(Float); quality_score_after=Column(Float); preprocessing_steps=Column(JSON); original_page_path=Column(String); preprocessed_page_path=Column(String); preprocessed_page_paths=Column(JSON); preprocessing_status=Column(String); preprocessing_error=Column(Text); created_at=Column(DateTime, default=datetime.utcnow)
