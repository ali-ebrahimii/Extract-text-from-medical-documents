...
NUM_RE=r'\d+(?:\.\d+)?'
UNIT_RE=r'(?:%|10\^\d+/uL|[A-Za-zµ]+(?:/[A-Za-zµ]+)?)'
REF_RE=r'(?:<\s*\d+(?:\.\d+)?|>\s*\d+(?:\.\d+)?|\d+(?:\.\d+)?\s*-\s*\d+(?:\.\d+)?)'
...
class LabExtractorV2:
    ...
    def extract(self,text:str,lines:list[str]|None=None,unsafe_ocr:bool=False)->list[LabResultV2]:
        ...
        for i,line in enumerate(clean_lines):
            ...
            if not name or name[0] in seen or i+3 >= len(clean_lines): continue
            ...
            rows.append(self.validator.validate(row,unsafe_ocr)); seen.add(std)
        for line in lines:
